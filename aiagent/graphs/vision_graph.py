...
class VisionRunner:
    ...
    def _build_chat_context_node(self,state:VisionGraphState) ->VisionGraphState:
        ...
        if not result:
            raise RuntimeError("result is not configued")

        character_lines:list[str] = []
        for item in result.recognized_characters[:3]:
            evidence = ";".join(item.evidence[4:])
            character_lines.append(
                f"- {item.name} ({item.character_id}), confidence={item.confidence:.3f}, evidence={evidence}"
            )

        if not character_lines:
            character_text = "未能高置信度确认具体角色"
        else:
        ...
        2. 如果 is_confident=false，不要强行断言角色身份。
        3. 如果识别到角色，但置信度不高，要用“像是 / 可能是 / 我不敢完全确定”这类保守表达。
        4. 不要编造图片里没有的信息。
        """.strip()
    ...
    def _build_memory_hint_node(self,state:VisionGraphState) ->VisionGraphState:
        ...
        if not result.memory.should_consider:
            ...
            memory_hint = (
                ...
                f"场景：{result.scene}\n"
                f"识别角色："
                f"{'、'.join([item.name for item in result.recognized_characters]) or '无'}"
            )
